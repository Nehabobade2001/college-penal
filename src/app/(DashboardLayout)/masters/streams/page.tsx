  ...
  return (
    <div className="p-6 main-dashboard">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-semibold text-white">Master - Streams</h1>
      ...
      <div>
        <h2 className='text-lg font-medium mb-2'>All Streams</h2>
        {loading ? (
          <div>Loading...</div>
        ) : (
          <div className='overflow-auto'>
            <table className='w-full table-auto border dashboard-table'>
              ...
              <tbody>
                {streams.length === 0 ? (
                  <tr><td colSpan={6} className='p-4'>No streams found.</td></tr>
                ) : (
                  streams.map((s, idx) => (
                    <tr key={s.id} className='border-t'>
                      ...
                      <td className='p-2'>{s.program?.name ?? (typeof s.program === 'object' ? (s.program?.name ?? JSON.stringify(s.program)) : s.program) ?? (programs.find(p => p.id === s.programId)?.name) ?? ''}</td>
